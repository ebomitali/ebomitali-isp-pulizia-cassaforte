...
/**
 * Spock specification for {@link JsonBuildMapClient}.
 *
 ...
 */
class JsonBuildMapClientSpec extends Specification {

    JsonBuildMapClient client

    def setup() {
        def cfg = new PuliziaCassaforteConfig(
            buildMapPath: new File(getClass().getResource('/fixtures/buildmap.json').toURI()).canonicalPath
        )
        client = new JsonBuildMapClient('ATO', cfg)
    }
    ...
    def "getGeneratedObjects returns empty list for unknown source path"() {
        expect:
        client.getGeneratedObjects('ATO/no/such/file.cbl') == []
    }
}
