...
// Sources merged: 24 files from '/Users/bomitalievelino/Documents/Workspace/isp-ibm-mauden/repo/pulizia-cassaforte/full-fat-source/build/temp-sources'
// ============================================================
 ...
 *   <li>{@code DbbCtxBuildMapClient}   — queries the live DBB build map in a DBB task context (not yet implemented).</li>
 * </ul>
 *
...
class BuildMapClientFactory {
     ...
     */
    static BuildMapClient fromJson(File bmFile, String buildGroupName) {
        log.debug("Creating local BuildMapClient from JSON: {}", bmFile)
        return new LocalBuildMapClient(bmFile.canonicalPath, buildGroupName)
    }
 ...
 */
@Slf4j
class LocalBuildMapClient extends BuildMapClient {
    private final List data

    LocalBuildMapClient(String jsonFilePath, String buildGroupName) {
...
class PuliziaCassaforteImpl {
    ...
    int doPuliziaCassaforte(File listToProcess, String environment, String buildGroup, Properties props) {
        ...
        if (buildMapClientType == 'db2') {
            ...
            this.buildMapClient = BuildMapClientFactory.fromJson(new File(buildMapPath), buildGroup)
        }
...
class QueryBuildMapImpl {
    ...
    int run(String listFile, String buildGroup, File bmFile) {
        return execute(listFile, buildGroup, BuildMapClientFactory.fromJson(bmFile, buildGroup))
    }
 ...
 */
@Slf4j
class ZosBuildMapClient extends BuildMapClient {

    private BuildGroup buildGroup

    private final String userId
    private final File pwFile
    private final Properties db2ConnectionProps
    ...
    ZosBuildMapClient(BuildGroup buildGroup) {
        ...
        this.userId = null
        this.pwFile = null
        this.db2ConnectionProps = null
        log.debug("ZosBuildMapClient initialized for group '{}'", buildGroupName)
    }

    /**
     * Deferred constructor: {@link BuildGroup} is resolved on first call to
     * {@link #getGeneratedObjects} by connecting to the DB2 metadata store.
     *
     * @param buildGroupName     Name of the DBB build group to look up.
     * @param userId             DB2 user ID.
     * @param pwFile             DB2 password file.
     * @param db2ConnectionProps DB2 connection properties (must contain {@code url}).
     */
    ZosBuildMapClient(String buildGroupName, String userId, File pwFile, Properties db2ConnectionProps) {
        this.buildGroup = null
        this.buildGroupName = buildGroupName
        this.userId = userId
        this.pwFile = pwFile
        this.db2ConnectionProps = db2ConnectionProps
        log.debug("ZosBuildMapClient configured for group '{}' (deferred connection)", buildGroupName)
    }
     ...
     * label convention used by DBB when recording build outputs.  Scope is fixed by the
     * group supplied at construction time.
     *
...
// --- DbbCtxBuildMapClient.groovy (script) ---
