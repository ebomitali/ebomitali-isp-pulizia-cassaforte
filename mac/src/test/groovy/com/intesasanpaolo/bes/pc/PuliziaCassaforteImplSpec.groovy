...
class PuliziaCassaforteImplSpec extends Specification {
    ...
    // ─── doPuliziaPostBuild tests ─────────────────────────────────────────

    def "doPuliziaPostBuild: ST environment requires predecessor cleanup"() {
        given:
        def props = new Properties()
        props.setProperty('buildMapClientType', 'json')
        props.setProperty('buildMapPath', bmFile.canonicalPath)
        ...
        expect:
        // Should return 0: runs without error (no member exists to delete, but no failure either)
        impl.doPuliziaPostBuild(SOURCE_PATH, 'ST', BUILD_GROUP, props) == 0
    }

    def "doPuliziaPostBuild: PR environment requires predecessor cleanup"() {
        given:
        def props = new Properties()
        props.setProperty('buildMapClientType', 'json')
        props.setProperty('buildMapPath', bmFile.canonicalPath)
        ...
        expect:
        impl.doPuliziaPostBuild(SOURCE_PATH, 'PR', BUILD_GROUP, props) == 0
    }

    def "doPuliziaPostBuild: ATO environment (no predecessor) returns 0 without error"() {
        given:
        def props = new Properties()
        props.setProperty('buildMapClientType', 'json')
        props.setProperty('buildMapPath', bmFile.canonicalPath)
        ...
        expect:
        impl.doPuliziaPostBuild(SOURCE_PATH, 'ATO', BUILD_GROUP, props) == 0
    }

    def "doPuliziaPostBuild: ATI environment (no predecessor) returns 0 without error"() {
        given:
        def props = new Properties()
        props.setProperty('buildMapClientType', 'json')
        props.setProperty('buildMapPath', bmFile.canonicalPath)
        ...
        expect:
        impl.doPuliziaPostBuild(SOURCE_PATH, 'ATI', BUILD_GROUP, props) == 0
    }

    def "doPuliziaPostBuild: missing sourceToProcess throws"() {
        given:
        def props = new Properties()
        props.setProperty('buildMapClientType', 'json')
        props.setProperty('buildMapPath', bmFile.canonicalPath)
        ...
        when:
        impl.doPuliziaPostBuild(null, 'ST', BUILD_GROUP, props)
