...
// Invocation: groovyz -cp ${DBB_BUILD}/groovy/pulizia-cassaforte/lib/pulizia-cassaforte.jar:${DBB_BUILD}/groovy/pulizia-cassaforte/lib/pulizia-cassaforte-zos.jar PuliziaCassaforte.groovy [--dbid <user>] [--dbpf <pwFile>] [--db2-config <db2Connection.conf>] [--bmf <buildmap.json>] <file-lista> <environment> <build-group>
...
if (positionalArgs.size() < 3) {
    System.err.println "Usage: PuliziaCassaforte.groovy [--dbid <user>] [--dbpf <pwFile>] [--db2-config <conf>] [--bmf <buildmap.json>] <file-lista> <environment> <build-group>"
    System.exit(1)
}
...
if (dbid && dbpf) {
    // Live DB2 mode — verify password file and resolve + verify db2 connection config
    ...
    errors = new PuliziaCassaforteImpl().run(listFile, environment, buildGroup, dbid, dbpf, db2ConfigFile)
} else {
    // JSON mode — resolve and verify build map file
    ...
    errors = new PuliziaCassaforteImpl().run(listFile, environment, buildGroup, bmFile)
}
